...
/** 处理流/包装流(相对于节点流更高级)
 * 1：隐藏了底层节点流的差异，并对外提供了更方便的输入、输出功能，让我们只关心高级流的操作
 * 2：使用处理流包装了节点流，程序直接操作处理流，让节点流与底层的设备做IO操作
 * 3：只需要关闭处理流即可
 *
 * 构建包装流的时候，需要传一个节点流
 *
 * 包装流如何区分：写代码的时候，创建对象的时候，需要传递另一个流对象
 * new 包装流(流对象);
 */
 ...
 * BufferedWriter
 *
 * 缓冲流的目的：使用操作流的时候，习惯定义一个char[] byte[]
 * 该数组其实就是一个缓冲区
 * int read() 每次都从磁盘文件中读取一个字节，直接操作磁盘文件性能极低
 *
 * 缓冲流的作用:
 * 每次把数据先写出到内存缓冲区中，当内存缓冲区大小满了8192个字节，此时才把缓冲区的数据写出给磁盘文件
 *
 */

/** 转换流:把字节流转换成字符流
 * InpuitStreamReader:把字节输入流转成字符输入流
 * OutputStreamWriter:把字节输出流转换成字符输出流
 */

/** 为什么有字节转字符流，没有字符转字节流
 * 字节流可以操作一切文件(纯文本文件/二进制文件)
 * 字符流是用来操作文本时时候用的，本身是对字节流的增强
 *
 *
 */
